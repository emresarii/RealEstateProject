<script setup lang="ts">
  ...
  onMounted(async() => {
       ...
       await appointStore.getAppointment(route.params.id)
      appointment.value = appointStore.getSingleApp
...
<template>
    ...
    <div class="appoint-form-wrapper">
      ...
      <form action="" @submit.prevent="handleSubmit" class="appoint-form">
        <div class="appoint-form-body">
          <div class="appoint-inputs-text">
            ...
            <input type="datetime-local" required v-model="appointment_date" :placeholder="appointment_date">
      ...
      <div class="initial-appoint-info">
        <h3>Randevu bilgileri</h3>
        <p v-if="appointment && appointment.fields">Müşteri: {{appointment.fields.contact_name[0]}} {{ appointment.fields.contact_surname[0] }}</p>
        <p v-if="appointment && appointment.fields">Emlakçı: {{appointment.fields.agent_name[0]}} {{ appointment.fields.agent_surname[0] }}</p>
        <p v-if="appointment && appointment.fields">Tarih: {{appointment.fields.appointment_date}}</p>
      </div>
